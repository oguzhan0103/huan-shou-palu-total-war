...
本轮证明原生属性写入、精确读回和作用域门，没有直接拍摄战斗演出或实际投球捕获失败。F7 QA 探针虽然注册成功，但本机另一已安装 Mod 同时占用 F7，本轮没有产生探针样本，因此不计为通过证据。实机还暴露了与 B2 无关的 companion commerce JSON 编码错误；该问题不计入 B2 成功，也没有导致 B2 字段读回失败，后续修复证据见相邻的 `build24575825-20260823-companion-commerce` 目录。
