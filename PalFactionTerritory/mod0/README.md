...
状态：**Build 24575825 当前源码已部署并通过安装审计；小型聚落帕鲁攻城已完成两轮 4/4 原生生成、战斗激活和回收实机验收；商人商会已完成七柜台自动驻场、七个势力商品行绑定、原生 ItemShop 连续开店及返回标题安全清理实机验收。集中测试快捷键与商业结算写入默认关闭。**
...
1. 商人商会机制、商品组绑定、`FTPoint90` 最终落位、生产自动驻场与返回标题安全清理已经实机通过；队形、朝向和自由漫步不作为 1.0 机制门槛。
2. 分别完成采购补差与限额商业好感度的最终实机交易验收；在此之前保持商品行、原生绑定、生成和刷货启用，补钱与好感度写入继续遵循服务器成功确认门槛。
...
- 当前部署经本地安装审计确认：49 个运行时文件与当前源码哈希一致；原始日志与部署快照保留在私有验收区，不进入公开仓库。
